...
abstract class DirtyChestMenu(
    ...
    fun pushItem(item: ItemStack, vararg slots: Int): ItemStack? {
        ...
        for (slot in slots) {
            if (left compareTo 0) return null
            ...
            if (stack.amount.compareTo(max) && ItemUtils.canStack(wrapper, stack)) {
        ...
        return if (left compareTo 0) ItemStack(item.type, left) else null
    }
