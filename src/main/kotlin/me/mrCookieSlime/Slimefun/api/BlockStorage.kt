...
import io.github.thebusybiscuit.slimefun4.api.items.SlimefunItem.Companion.getByItem
import io.github.thebusybiscuit.slimefun4.implementation.Slimefun
import io.github.thebusybiscuit.slimefun4.implementation.Slimefun.Companion.cfg
import io.github.thebusybiscuit.slimefun4.implementation.Slimefun.Companion.getBlockDataService
import io.github.thebusybiscuit.slimefun4.implementation.Slimefun.Companion.getRegistry
import io.github.thebusybiscuit.slimefun4.implementation.Slimefun.Companion.instance
...
import io.github.thebusybiscuit.slimefun4.implementation.Slimefun.Companion.tickerTask
...
class BlockStorage(private val world: World) {
    ...
    init {
        ...
        if (getRegistry().worlds.putIfAbsent(world.name, this) != null) {
            error("Attempted to create duplicate BlockStorage for world '${world.name}'")
        }

        logger()!!.log(Level.INFO, "Loading Blocks for World \"{0}\"", world.name)
        logger()!!.log(Level.INFO, "This may take a long time...")

        val dir = File(PATH_BLOCKS + world.name)

        if (dir.exists()) loadBlocks(dir) else dir.mkdirs()
        loadChunks()
        if (!instance()!!.isUnitTest) loadInventories()
    }
    ...
    private fun loadBlocks(dir: File) {
            ...
            when {
                ...
                file.name.endsWith(".sfb") -> {
                    if (System.currentTimeMillis() > timestamp + delay) {
                        ...
                        logger()!!.info("Loading Blocks... $progress% done (\"${world.name}\")")
        ...
        logger()!!.info("Loading Blocks... 100% (FINISHED - ${time}ms)")
        logger()!!.info("Loaded a total of $totalBlocks Blocks for World \"${world.name}\"")
        if (totalBlocks > 0) {
            logger()!!.info("Avg: ${NumberUtils.roundDecimalNumber(time.toDouble() / totalBlocks)}ms/Block")
        }
    }

    private fun logCorruptedFile(file: File) {
        logger()!!.warning("File with corrupted blocks detected!")
        logger()!!.warning("Slimefun will simply skip this File, you should look inside though!")
            ...
            try {
                parseBlockInfo(loc, cfg.getString(key))?.takeIf { it.contains("id") }?.let { blockInfo ->
                    if (storage.putIfAbsent(loc, blockInfo) != null) {
                        if (getRegistry().logDuplicateBlockEntries()) {
                            logger()!!.info(
                    ...
                    if (getRegistry().tickerBlocks.contains(fileName)) {
                        tickerTask.enableTicker(loc)
                    }
                }
            } catch (x: Exception) {
                logger()!!.log(Level.WARNING, x) {
                    "Failed to load ${file.name}($key) for Slimefun $version"
                }
            }
        }
    }
            ...
            cfg.getKeys(false)
                ...
                .forEach { key ->
                    cfg.getString(key)?.let { json ->
                        getRegistry().chunks[key] = BlockInfoConfig(parseJSON(json))
                    }
                }

        } catch (x: Exception) {
            logger()!!.log(Level.WARNING, x) {
                "Failed to load ${chunks.name} in World ${world.name} for Slimefun ${version}"
            }
        }
    }
        ...
        File(PATH_INVENTORIES).listFiles()?.forEach { file ->
            ...
            deserializeLocation(file.nameWithoutExtension)?.takeIf { it.world == world }?.let { loc ->
                try {
                    ...
                    logger()!!.log(Level.SEVERE, x) { "Error loading inventory: ${file.name}" }
                }
            }
        }
        ...
        File("data-storage/Slimefun/universal-inventories").listFiles()?.forEach { file ->
            ...
            try {
                ...
                BlockMenuPreset.getPreset(cfg.getString("preset"))?.let { preset ->
                    getRegistry().universalInventories[preset.id] = UniversalBlockMenu(preset, cfg)
                }
            } catch (x: Exception) {
                logger()!!.log(Level.SEVERE, x) { "Error loading universal inventory: ${file.name}" }
            }
        }
    }
    ...
    fun computeChanges() {
        changes = blocksCache.size + inventories.values.sumOf { it.unsavedChanges } +
                getRegistry().universalInventories.values.sumOf { it.unsavedChanges }
    }
    ...
    fun save() {
        ...
        logger()!!.info("Saving block data for world \"${world.name}\" ($changes change(s) queued)")
            ...
            when {
                cfg.getKeys()!!.isEmpty() -> cfg.getFile()?.takeIf { it.exists() }?.delete()
                    ...
                    runCatching { Files.move(tmp.toPath(), cfg.getFile()?.toPath()!!, StandardCopyOption.ATOMIC_MOVE) }
                        .onFailure {
                            logger()!!.severe("Error copying temp file for Slimefun $version")
                        }
                }
            }
        }
        ...
        getRegistry().universalInventories.values.forEach { it.save() }
        changes = 0
    }
    ...
    fun clearInventory(l: Location) {
        ...
        menu.toInventory()?.viewers?.toList()?.forEach { runSync(it::closeInventory) }
    ...
    companion object {
        ...
        fun getStorage(world: World): BlockStorage? = getRegistry().worlds[world.name]
        ...
        private fun deserializeLocation(l: String?): Location? =
            CommonPatterns.SEMICOLON.split(l).takeIf { it.size == 4 }?.let { parts ->
                ...
                logger()!!.warning("Could not parse Number from: $l")
                null
            }
        ...
        @JvmStatic
        fun store(block: Block, item: ItemStack) = getByItem(item)?.let { addBlockInfo(block, "id", it.id, true) }
        ...
        fun setBlockInfo(l: Location, cfg: Config, updateTicker: Boolean) {
            ...
            when {
                BlockMenuPreset.isUniversalInventory(id) -> {
                    getRegistry().universalInventories.getOrPut(id) { UniversalBlockMenu(preset) }
                }
        ...
        @JvmStatic
        fun clearBlockInfo(l: Location, destroy: Boolean = true) = tickerTask.queueDelete(l, destroy)
        ...
        fun clearAllBlockInfoAtChunk(world: World, chunkX: Int, chunkZ: Int, destroy: Boolean) {
            ...
            tickerTask.queueDelete(toClear)
        }
        ...
        fun deleteLocationInfoUnsafely(l: Location, destroy: Boolean) {
            ...
            if (destroy) {
                ...
                tickerTask.disableTicker(l)
            }
        }

        @JvmStatic
        fun moveBlockInfo(from: Location, to: Location) = tickerTask.queueMove(from, to)
        ...
        private fun parseBlockInfo(l: Location, json: String?): BlockInfoConfig? {
            try {
                ...
                logger.log(
                    Level.SEVERE,
                    x
                ) { "An Error occurred while parsing Block Info for Slimefun $version" }
                return null
            }
        }
        ...
        private fun serializeBlockInfo(cfg: Config): String? {
            ...
            try {
                ...
                logger()!!.log(Level.SEVERE, "An error occurred while serializing BlockInfo", x)
                return null
            }
        }
        ...
        fun check(b: Block): SlimefunItem? {
            ...
            return if (id == null) null else getById(id)
        }
        ...
        fun checkID(b: Block): String? {
            ...
            if (Bukkit.isPrimaryThread() && getBlockDataService().isTileEntity(b.getType())) {
                val blockData = getBlockDataService().getBlockData(b)
        ...
        fun isWorldLoaded(world: World): Boolean {
            return getRegistry().getWorlds().containsKey(world.getName())
        }

        @JvmStatic
        fun hasUniversalInventory(id: String?): Boolean {
            return getRegistry().getUniversalInventories().containsKey(id)
        }
        ...
        fun getUniversalInventory(id: String?): UniversalBlockMenu? {
            return getRegistry().getUniversalInventories().get(id)
        }
        ...
        fun getChunkInfo(world: World, x: Int, z: Int): Config {
            try {
                ...
                var cfg = getRegistry().chunks.get(key)

                if (cfg == null) {
                    cfg = BlockInfoConfig()
                    getRegistry().chunks.put(key, cfg)
                }

                return cfg
            } catch (e: Exception) {
                logger()!!.log(Level.SEVERE, e) { "Failed to parse ChunkInfo for Slimefun " + version }
                return emptyBlockData
            }
        }
        ...
        fun setChunkInfo(world: World, x: Int, z: Int, key: String?, value: String?) {
            ...
            var cfg = getRegistry().chunks.get(serializedChunk)

            if (cfg == null) {
                cfg = BlockInfoConfig()
                getRegistry().chunks.put(serializedChunk, cfg)
            }
        ...
        fun hasChunkInfo(world: World, x: Int, z: Int): Boolean {
            val serializedChunk: String = serializeChunk(world, x, z)
            return getRegistry().chunks.containsKey(serializedChunk)
        }
