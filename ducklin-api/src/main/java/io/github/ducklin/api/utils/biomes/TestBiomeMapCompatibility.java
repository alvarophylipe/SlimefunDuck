package io.github.ducklin.api.utils.biomes;
...
import io.github.ducklin.api.utils.JsonUtils;
...
class TestBiomeMapCompatibility {
    ...
    private static void loadBiomes(@Nonnull MinecraftVersion version) {
        String path = "/biomes/" + version.name + ".json";
    ...
    private void checkBiomes(JsonArray biomes, JsonArray allBiomes, MinecraftVersion version) {
        for (JsonElement biome : biomes) {
             ...
             */
            Assertions.assertTrue(allBiomes.contains(biome), "Biome \"" + biome.getAsString() + "\" does not exist on Minecraft " + version.name);
        }
    }
